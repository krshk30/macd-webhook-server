...
function isDuplicate(ticker, action) {
    const key = `${ticker}:${action}`, now = Date.now(), last = recentAlerts.get(key);
    if (last && (now - last) < DEDUP_WINDOW_MS) { log('WARN', `Duplicate filtered: ${key}`); return true; }
...
function isWithinTradingHours() {
    ...
    const h = eastern.getHours(), m = eastern.getMinutes();
    const s = parseInt(process.env.TRADING_START_HOUR || '7'), e = parseInt(process.env.TRADING_END_HOUR || '18');
    log('DEBUG', `Trading hours: ${h}:${m.toString().padStart(2,'0')} ET (${s}:00-${e}:00)`);
    return h >= s && h < e;
}
...
function scalePosition(ticker, sellPct, reason, price) {
    const p = positions.get(ticker); if (!p) return null;
    const sell = Math.min(Math.floor(p.initialQuantity * (sellPct / 100)), p.remainingQuantity);
    if (sell <= 0) { log('WARN', `Scale ${ticker}: 0 shares`); return null; }
    p.remainingQuantity -= sell; p.soldPct += sellPct;
    p.scaledExits.push({ reason, shares: sell, price, time: new Date().toISOString() });
    const pnl = (price - p.entryPrice) * sell; dailyPnL += pnl;
    log('POSITION', `Scaled ${ticker}: sold ${sell}/${p.initialQuantity} (${reason}) | remaining: ${p.remainingQuantity} | P&L: $${pnl.toFixed(2)}`);
    return { sharesToSell: sell, pnl };
}
function closePosition(ticker, exitPrice, reason) {
    const p = positions.get(ticker); if (!p) return null;
    const rem = p.remainingQuantity, pnl = (exitPrice - p.entryPrice) * rem; dailyPnL += pnl;
    const summary = { ticker, entryPrice: p.entryPrice, exitPrice, totalShares: p.initialQuantity, remainingClosed: rem, reason, pnl, scaledExits: p.scaledExits };
    positions.delete(ticker);
    log('POSITION', `Closed ${ticker}: ${rem} shares @ $${exitPrice.toFixed(2)} | P&L: $${pnl.toFixed(2)} | ${reason}`);
    return summary;
}
...
function getStatus() {
    ...
    return { openPositions: Array.from(positions.values()).map(p => ({ ticker: p.ticker, entryPrice: p.entryPrice, remaining: p.remainingQuantity, soldPct: p.soldPct, enteredAt: p.enteredAt, isOrphan: p.isOrphan })),
        positionCount: positions.size, dailyPnL: dailyPnL.toFixed(2), dailyTradeCount, maxPositions: parseInt(process.env.MAX_POSITIONS || '3') };
}
