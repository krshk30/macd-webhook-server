/**
 * Position Tracking Service v1.2.2
 */
...
function canOpenPosition(ticker) {
    ...
    if (positions.size >= maxPos) return { allowed: false, reason: `Max positions reached (${positions.size}/${maxPos})` };
    const maxLoss = parseFloat(process.env.MAX_DAILY_LOSS || '-500');
    if (dailyPnL <= maxLoss) return { allowed: false, reason: `Daily loss limit reached ($${dailyPnL.toFixed(2)})` };
...
function getOrphans(timeoutMins) {
    ...
    for (const pos of positions.values()) {
        if (pos.isOrphan && pos.orphanDetectedAt) {
            if ((now - new Date(pos.orphanDetectedAt).getTime()) / 60000 >= timeoutMins) result.push(pos);
        }
    }
    return result;
}
...
function closePosition(ticker, exitPrice, reason) {
    ...
    const summary = {
        ...
        reason, pnl, holdTime: new Date() - new Date(pos.enteredAt),
...
function getStatus() {
    ...
    return {
        ...
        dailyPnL: dailyPnL.toFixed(2), dailyTradeCount,
