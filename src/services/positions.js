/**
 * Position Tracking Service v1.2.4
 */
...
function resetDailyIfNeeded() {
    ...
    if (today !== lastResetDate) { dailyPnL = 0; dailyTradeCount = 0; lastResetDate = today; log('INFO', 'Daily P&L reset'); }
}
...
function isWithinTradingHours() {
    ...
    log('DEBUG', `Trading hours: ${hour}:${minute.toString().padStart(2,'0')} ET (${startHour}:00-${endHour}:00)`);
...
function canOpenPosition(ticker) {
    ...
    if (positions.size >= maxPos) return { allowed: false, reason: `Max positions (${positions.size}/${maxPos})` };
    const maxLoss = parseFloat(process.env.MAX_DAILY_LOSS || '-500');
    if (dailyPnL <= maxLoss) return { allowed: false, reason: `Daily loss limit ($${dailyPnL.toFixed(2)})` };
...
function openPosition(ticker, entryPrice, quantity) {
    positions.set(ticker, {
        ticker, entryPrice, initialQuantity: quantity, remainingQuantity: quantity,
        enteredAt: new Date().toISOString(),
        hit2pct: false, hitFast4pct: false, hit4after2: false,
        soldPct: 0, scaledExits: [],
        isOrphan: false, orphanDetectedAt: null
    });
    dailyTradeCount++;
...
function markOrphan(ticker) {
    ...
    if (pos) { pos.isOrphan = true; pos.orphanDetectedAt = new Date(); log('WARN', `Marked ${ticker} as orphan`); }
}
function getOrphans(timeoutMins) {
    const result = [];
    const now = Date.now();
    for (const pos of positions.values()) {
        if (pos.isOrphan && pos.orphanDetectedAt && (now - new Date(pos.orphanDetectedAt).getTime()) / 60000 >= timeoutMins)
            result.push(pos);
    }
    return result;
}
function scalePosition(ticker, sellPct, reason, currentPrice) {
    ...
    if (actualSell <= 0) {
        log('WARN', `Scale ${ticker}: 0 shares (${sellPct}% of ${pos.initialQuantity})`);
        return null;
    }
    ...
    dailyPnL += scalePnL;
    log('POSITION', `Scaled ${ticker}: sold ${actualSell}/${pos.initialQuantity} (${reason}) | remaining: ${pos.remainingQuantity} | P&L: $${scalePnL.toFixed(2)}`);
...
function closePosition(ticker, exitPrice, reason) {
    ...
    dailyPnL += pnl;
    const summary = {
        ticker, entryPrice: pos.entryPrice, exitPrice,
        totalShares: pos.initialQuantity, remainingClosed: remaining,
        reason, pnl, holdTime: new Date() - new Date(pos.enteredAt), scaledExits: pos.scaledExits
    };
...
function getStatus() {
    ...
    return {
        openPositions: Array.from(positions.values()).map(p => ({
            ticker: p.ticker, entryPrice: p.entryPrice, remaining: p.remainingQuantity,
            soldPct: p.soldPct, enteredAt: p.enteredAt, isOrphan: p.isOrphan
        })),
        positionCount: positions.size, dailyPnL: dailyPnL.toFixed(2), dailyTradeCount,
        maxPositions: parseInt(process.env.MAX_POSITIONS || '3'), maxDailyLoss: process.env.MAX_DAILY_LOSS || '-500'
    };
}
module.exports = {
    positions: {
        isDuplicate, canOpenPosition, openPosition, getPosition,
        scalePosition, closePosition, markMilestone, getStatus,
        markOrphan, getOrphans
    }
};
