/**
 * Webhook Route v1.2.2 — receives TradingView alerts
 * Passes price for extended hours LIMIT orders
 */
...
router.post('/webhook', async (req, res) => {
    ...
    if (body.token !== process.env.WEBHOOK_TOKEN) {
        log('WARN', `Unauthorized webhook from ${req.ip}`);
        return res.status(401).json({ error: 'unauthorized' });
    }
    if (!schwabService.isAuthenticated()) {
        log('ERROR', 'Not authenticated — order rejected');
...
async function handleBuy(ticker, price, body) {
    ...
    return {
        success: result.success, action: 'BUY', ticker, quantity: qty,
        entryPrice, tp: tpPrice.toFixed(2), sl: slPrice.toFixed(2),
...
async function handleScale(ticker, price, body) {
    ...
    if (!scaleResult || scaleResult.sharesToSell <= 0) return { success: false, rejected: 'nothing to sell' };
