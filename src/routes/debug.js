...
router.get('/schwab', async (req, res) => {
    ...
    try {
        results.userPreference = { status: (await axios.get('https://api.schwabapi.com/trader/v1/userPreference', { headers: h, timeout: 10000 })).status, ok: true };
    } catch (e) { results.userPreference = { status: e.response?.status || 'ERR', ok: false }; }
    ...
    } catch (e) { results.accountNumbers = { status: e.response?.status || 'ERR', ok: false }; }

    try {
        results.accounts = { status: (await axios.get('https://api.schwabapi.com/trader/v1/accounts', { headers: h, timeout: 10000 })).status, ok: true };
    } catch (e) { results.accounts = { status: e.response?.status || 'ERR', ok: false }; }

    try {
        results.marketData = { status: (await axios.get('https://api.schwabapi.com/marketdata/v1/quotes?symbols=AAPL&fields=quote', { headers: h, timeout: 10000 })).status, ok: true };
    } catch (e) { results.marketData = { status: e.response?.status || 'ERR', ok: false }; }

    let diagnosis = '⚠️ Issue';
