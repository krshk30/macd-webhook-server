...
router.get('/schwab', async (req, res) => {
    const t = schwabService.getAccessToken(); if (!t) return res.json({ diagnosis: 'NO TOKEN', fix: '/auth/start' });
    const h = { 'Authorization': `Bearer ${t}`, 'Accept': 'application/json' };
    const results = {}; log('INFO', 'Debug...');
    ...
    if (results.accountNumbers.ok && results.accountNumbers.data?.length > 0) { schwabService.setAccountHash(results.accountNumbers.data[0].hashValue); diagnosis = '✅ ALL WORKING'; }
    log('INFO', `Debug: ${diagnosis}`);
    res.json({ timestamp: new Date().toISOString(), diagnosis, accountHash: schwabService.getAccountHash()?.substring(0,10) + '...' || null, tokenStatus: schwabService.getTokenStatus(), session: schwabService.getSessionType(), results });
});
