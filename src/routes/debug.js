...
router.get('/schwab', async (req, res) => {
    ...
    if (results.accountNumbers.ok && results.accountNumbers.data?.length > 0) { schwabService.setAccountHash(results.accountNumbers.data[0].hashValue); }
    res.json({ timestamp: new Date().toISOString(), diagnosis: results.accountNumbers.ok ? '✅ ALL WORKING' : '⚠️ Issue', accountHash: schwabService.getAccountHash()?.substring(0,10) + '...' || null, tokenStatus: schwabService.getTokenStatus(), session: schwabService.getSessionType(), results });
});
